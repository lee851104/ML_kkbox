...
> ### 🚧 專案狀態：M0–M3 完成，M4 未開始
>
> 已完成：全部 10 個競賽檔案下載並實測、`src/` 全部資料／特徵／模型模組、
> EDA 與 8 張圖表、M1 baseline、M2 收聽特徵聚合、M3 四個實驗、
> 一次獨立洩漏審查（7 個問題全數修正）、
> 一次反向時間外驗證（推翻了 M3 的選模結論）、
> 測試 **81 passed · 1 skipped**（僅紅線 4 待 M6）、Makefile、GitHub Actions CI。
>
...
| M1 | LightGBM（交易 + 用戶屬性，23 特徵） | **0.16305** | **−47.0%** | ✅ 已實測 |
| M2 | ＋ 收聽行為聚合特徵（+38 特徵，共 61） | **0.15910** | **−2.4%** | ✅ 已實測 |
| M3 | LightGBM（四個實驗皆無可信改善，見下方） | **0.15910** | **0.00%** | ✅ 已實測 |
| M4 | ＋ 機率校準 | — | — | ⬜ |
| M6 | 部署版（提前 7 天預測） | — | — | ⬜ |

M2 的 Feb cohort 內部 5-fold：**0.08558 ± 0.00084**。與時間外分數的差距見下方「M1 的發現」。

⚠️ **M1–M3 的分數於 2026-08-08 全部重測** —— 修掉一個「cutoff 自己來自未來」的洩漏（見下方「M3 期間修掉的 7 個洩漏風險」）。舊數字作廢。
...
模型在 Feb 內部**極度穩定**（標準差只有 0.00084），但換到下一個月的 cohort 就掉了將近一倍。
...
|---|---|---|---|---|---|
| 新進用戶 | 89,266 | 9.2% | 39.84% | **26.75%** | **0.5374** |
| 重複用戶 | 881,693 | 90.8% | 5.87% | **4.51%** | 0.1251 |
| 全體 | 970,959 | 100% | 8.99% | **6.56%** | 0.1630 |

預測平均 6.56%、實際 8.99% —— 模型學到的是訓練集 Feb 的基礎流失率 6.39%。
...
|---|---|---|---|---|
| `txn_only`（M1 對照） | 23 | 0.16305 | — | — |
| **`log_only`（無交易特徵）** | 38 | **0.29746** | **+82.4%** | — |
...
| `all`（M2 對照） | 61 | 0.15910 | −2.42% | 0.064% |

**結論一：收聽行為沒有獨立訊號。** `log_only` 是 0.29746，而「對所有人預測同一個數字」的常數基準是 0.30746 —— **38 個特徵、400 輪，只贏了 3.3%**。收聽資料單獨使用幾乎等於什麼都不知道。

**結論二：四組高度重疊。** 各組單獨貢獻相加是 7.12%，全部一起卻只有 2.42% —— **重疊掉三分之二**。四組其實都在量同一件事：「這個人還在不在用」。這給 M3 的 null importance 篩選一個明確動機。

**結論三：EDA 的預測是錯的，而這是最有價值的一課。**

根據 M0 的 EDA，我預期 **trend 最強**（它是唯一在兩個 cohort 方向一致的），**intensity 最弱**（它在 Mar 完全平坦）。實測**完全相反** —— intensity 最強（−2.71%），而最弱的是 recency（−0.77%）。
...
依消融結果，**繼續加收聽特徵的期望報酬很低**。0.159 → 0.108 的差距要靠 M3 的調參、模型比較與特徵篩選。
...
## M3 的發現：四個實驗，沒有一個帶來可信的改善

M3 問的是「還有多少分數可以從建模這一側榨出來」。四個實驗、約 130 次訓練，答案是 **0**：兩個明確變差，兩個看似小賺但撐不過穩健性檢驗。

分界線不是做了什麼，是**有沒有在訓練集內部做選擇**——而唯一看起來例外的那個（換套件），在反向驗證裡也倒了。

| 實驗 | 指令 | 最佳 Mar log loss | 相對 M2（0.15910） |
|---|---|---|---|
| 三方模型比較 | `make compare` | 0.15781（CatBoost） | −0.81%　⚠️ 未通過反向驗證 |
...
### CatBoost 在正向勝出，但這個結論撐不住

| 模型 | 輪數 | Mar log loss | 重複用戶 | 新進用戶 | 秒 |
|---|---|---|---|---|---|
| **CatBoost** | 1958 | **0.15781** | 0.12062 | **0.52509** | 292 |
| 三者平均 | — | 0.15796 | 0.12078 | 0.52521 | — |
| LightGBM | 667 | 0.15910 | 0.12137 | 0.53180 | **13** |
| XGBoost | 607 | 0.16082 | 0.12268 | 0.53748 | 34 |

三家拿到**完全相同**的 61 個特徵、相同的切分、相同的 early stopping 驗證集（[src/models/compare.py](src/models/compare.py)），三家皆未調參。

**CatBoost 的優勢集中在新進用戶**（贏 LightGBM 1.3%，重複用戶只贏 0.6%）。那群人交易史短、類別特徵的相對權重高，而 CatBoost 的原生類別處理正是為此設計的。

**但這個 0.81% 沒有通過反向驗證**（見下一節）：換成 Mar 訓練、Feb 驗證之後，CatBoost 與 LightGBM 的差距掉到 0.93σ，而它與「三者平均」直接換位。加上它慢 22 倍，**最終採用 LightGBM**。
...
| 門檻 | 特徵數 | Feb-sel | **Mar** |
|---|---|---|---|
| 全特徵 | 61 | 0.07979 | **0.15864** |
| > null p99（凍結） | 35 | **0.07951** | **0.15954** |
| | | −0.35% | **+0.57%** |

**Feb 內部改善，時間外退步。方向相反。**

被砍掉的 26 個裡有 22 個是收聽特徵 —— 而 M2 的消融已經測到收聽特徵在 Mar 上確實有 2.42% 的貢獻。「在 Feb 內部贏不過自己的雜訊」與「在 Mar 上沒用」是兩件事。

**結論：61 個特徵全留。** 若 M6 為了推論成本要砍到 35 個，代價是 +0.57%，那是工程取捨不是模型取捨。
...
| 變體 | Feb 內部 | Mar 時間外 |
|---|---|---|
| A 原生類別（對照） | 0.07972 | 0.15910 |
| **B OOF TE · `last_payment_method_id`** | 0.07981 | **0.15807** |
| C Naive TE · `last_payment_method_id` ⚠️ | 0.07981 | 0.15811 |
| D Naive TE · `msno`（切分後）⚠️ | 0.22132 | 0.28488 |
| **E Naive TE · `msno`（切分前）⚠️** | **0.00000** | **1.33325** |

**E 是教科書等級的災難**：先對整個訓練 cohort 做編碼、之後才切驗證集，內部 log loss 完美到 0.00000，時間外 1.33325 —— 比「對所有人猜同一個機率」的 0.30746 還差 4.3 倍。

**C 才是最有價值的發現：同樣的違規在低基數欄位上量不出來。** `last_payment_method_id` 只有 33 種取值、每種平均兩萬多列，自己那一列的標籤只佔 1/24000，分數與合規的 B 幾乎相同（0.15811 vs 0.15807）。

**所以紅線的門檻必須訂在「作法」而不是「分數有沒有變差」** —— 靠看分數抓不到 C，而同一段程式碼換到高基數欄位就是 E。

### 調參：Feb 上賺 0.78%，Mar 上賠 0.99%
...
| 最佳 trial | **0.07917** | **0.16022** |
| 變化 | −0.78%（改善） | **+0.99%（退步）** |

**符號直接反轉。** 最佳組合的方向是「更慢、更深」（`learning_rate` 0.05 → 0.018、`num_leaves` 31 → 203）—— 也就是**更貼合 Feb**。容量更大的模型把 Feb 的分布學得更細，而那些細節在 Mar 上不成立。

**結論：沿用未調參的設定。** 搜尋結果保留在 `best_params.yaml` 供追溯，但不採用。
...
|---|---|---|
| 三方模型比較 | 無 | **−0.81%** |
| Target encoding | 無 | **−0.65%** |
| Null importance 篩選 | **有** | +0.57% |
| 超參數搜尋 | **有** | +0.99% |

**兩個做了 Feb 內部選擇的實驗，兩個都在 Mar 上退步。** Feb 與 Mar 的流失率是 6.39% 與 8.99%，組成也不同 —— 任何依 Feb 內部分數挑出來的東西，挑的是「最合 Feb 胃口」，而那正是最不容易外推的方向。
...
**判定不能只看排名**——那會把「差 0.1% 的兩個選項換位」跟「真正的翻轉」當成同一件事。所有差距都除以 **σ = 0.00084**（實測的 5-fold 標準差）：
...
**結論：M3 宣稱的「CatBoost 勝出 −0.81%」不成立。** 整張比較表擠在 2–4 個標準差之內；唯一站得住的是「XGBoost 明顯最差」，而那對選模沒有幫助。

**因此 M6 的模型選擇改以成本為準，採用 LightGBM**——它與第一名的差距不可信，但它比 CatBoost **快 22 倍**。在一個差距不可信的比較裡，唯一可信的差異就是成本。

這也讓 M3 的結論更乾淨：**四個實驗沒有任何一個帶來可信的分數改善。** 分數的天花板確實不在模型這一側。
...
應為 **81 passed · 1 skipped**，約 80 秒（唯一 skip 的是紅線 4，阻塞里程碑為 M6）。資料尚未下載時測試會 skip 而非 fail —— 剛 clone 完 repo 的人不該看到滿螢幕紅字。
...
**CI runner 上沒有原始資料** —— 資料依競賽規則不進 Git。因此 82 條測試裡有 35 條在 CI 上會被跳過。
