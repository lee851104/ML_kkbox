...
> 測試 **152 passed · 1 skipped**（僅紅線 4 待 M6）、Makefile、GitHub Actions CI。
>
...
> ⛔ **下表為「排序修正前」的數字，已作廢且尚未重跑**（`make encode` 不在
> 2026-08-10 的重測清單裡）。保留是因為本節的論證靠**變體之間的相對關係**
> （E 崩盤、C 與 B 難以分辨），那個關係不受基準位移影響。
...
| Target encoding | ⛔ **待重測**，不在新基準的重測清單裡 |
...
|---|---|---|---|---|
| Feb-sel（校準器 fit 在這裡） | 6.3913% | 6.4120% | **+0.33%** | **0.00038** |
| Mar cohort（評估） | 8.9941% | 6.5530% | **−27.14%** | 0.02441 |
...
| 相對偏差 | −27.14% | −27.40% |
| ECE | 0.02441 | 0.02464 |
| **log loss** | **0.15645** | **0.17172（+9.8%）** |
| 新進用戶 log loss | 0.53525 | **0.62313（+16.4%）** |
...
isotonic 把失準的區段壓平成同一個值。被壓平的是**機率最低的 59%**，於是
「投放給機率最高的前 K%」在 **K > 41% 之後不再有定義** —— 門檻落在那個區塊裡，
等於要從 57 萬個完全同分的人裡挑出誰該投放。K < 41% 也只是勉強可用：剩下的
39.8 萬人只有 605 個相異等級。**不是精度變差，是那條核心交付曲線的橫軸有一段
不存在了。**
...
0.94465 掉到 0.93634，意思是被壓平的區段大多原本**排得對**。這條原本寫成
...
**CI runner 上沒有原始資料** —— 資料依競賽規則不進 Git。因此 153 條測試裡有 35 條在 CI 上會被跳過。
...
- **所有業務金額系統性偏低約 27.1%，方向已知、本專案的資料修不掉。** 成因是 cohort 之間的基準率漂移（Feb 6.39% → Mar 8.99%），而校準器只能 fit 在已知標籤的月份上，看不到下一個月的漂移 —— 實測套上去反而讓 log loss 惡化 9.8%（見上方「M4 的發現」）。要修掉它需要一塊與部署期同分布的標籤資料，而 Apr cohort 的標籤是 Kaggle 測試集，本地取不到。
