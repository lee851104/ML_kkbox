...
> 一次獨立洩漏審查（7 個問題全數修正）、
> 測試 **74 passed · 1 skipped**（僅紅線 4 待 M6）、Makefile、GitHub Actions CI。
>
...
| M3 | CatBoost（同一份 61 特徵；篩選與調參皆使分數變差） | **0.15781** | **−0.81%** | ✅ 已實測 |
| M4 | ＋ 機率校準 | — | — | ⬜ |
| M6 | 部署版（提前 7 天預測） | — | — | ⬜ |
...
## M3 的發現：在 Feb 上最佳化，在 Mar 上賠錢

M3 問的是「還有多少分數可以從建模這一側榨出來」。四個實驗、約 130 次訓練，答案比預期更有意思：**兩個實驗小賺，兩個實驗賠錢，而分界線不是做了什麼，是有沒有在訓練集內部做選擇。**
...
**但 0.81% 的分數要付 22 倍的訓練時間。** 這個取捨留給 M6 —— 值不值得取決於服務多久重訓一次，不取決於分數本身。
...
應為 **74 passed · 1 skipped**，約 80 秒（唯一 skip 的是紅線 4，阻塞里程碑為 M6）。資料尚未下載時測試會 skip 而非 fail —— 剛 clone 完 repo 的人不該看到滿螢幕紅字。
...
**CI runner 上沒有原始資料** —— 資料依競賽規則不進 Git。因此 75 條測試裡有 35 條在 CI 上會被跳過。
