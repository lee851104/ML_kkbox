...
> |---|---|---|
> | M2 LightGBM | ~~0.15821~~ | **0.15705** |
> | M3 CatBoost | ~~0.15685~~ | **0.15367**（−2.16% vs LightGBM） |
> | M3 XGBoost | ~~0.15949~~ | **0.15726** |
>
> 成因與逐條結論對照見 [SPEC.md §7.11](SPEC.md)；完整執行紀錄見
> `reports/rebaseline/`。**唯一翻轉的結論**是 LightGBM vs XGBoost 由「可信」
> 變成「雜訊」；CatBoost 的優勢反而從 2.83σ 強化到 7.05σ。

| 階段 | 模型 | 時間外驗證 Log Loss | 相對前一步 | 狀態 |
|---|---|---|---|---|
| 基準 | 常數預測（訓練集流失率 6.3923%） | **0.30746** | — | ✅ 已實測 |
| M1 | LightGBM（交易 + 用戶屬性，23 特徵） | ~~0.16299~~ | ~~−47.0%~~ | ⛔ 待重測 |
| M2 | ＋ 收聽行為聚合特徵（+38 特徵，共 61） | ~~0.15821~~ → **0.15705** | **−48.9% vs 基準** | ✅ 已重測 |
| M3 | **CatBoost**（四個實驗只有換套件通過驗證） | ~~0.15685~~ → **0.15367** | **−2.16% vs LightGBM** | ✅ 已重測 |
