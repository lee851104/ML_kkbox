...
> ### 🚧 專案狀態：M0–M5 完成（M1–M4 三度重測後定版），下一步 M6 服務化
>
...
| **M5 原因碼** | SHAP 歸因（CatBoost 原生 TreeSHAP） | — | **名單 48,853 人 · 逐人 Top-3** | ✅ 已產出，見「M5 的原因碼」。⚠️ 其中 48.80% 的人有一句撐不到 M6 的 T−7 版本 |
| M6 | 部署版（提前 7 天預測） | — | — | ⬜ |
...
```
第 19,542 位　流失機率 0.9620
  1. 到期前最後一筆交易是取消　（取消紀錄，貢獻 +10.328）
  2. 近 7 天活躍天數 0 天　（收聽頻率，貢獻 +0.131）
  3. 近 7 天活躍天數是近 30 天日均的 0.00 倍　（收聽趨勢，貢獻 +0.099）
```
...
**這句原因碼不可沿用**。實測影響 **48.80% 的人、41.99% 的解釋強度**。
...
│   ├── ✅ explain/reasons.py     # M5 句型 + 語意分組 + 量測時點標註
...
├── ✅ reports/figures/           # 14 張圖表（01–07 + 03b 為 EDA，09–11 M4 校準，12–13 M4 業務指標，14 M5 原因碼）
├── ✅ reports/explanations/      # M5 的 manifest.json（進 Git）＋ 名單 CSV（不進 Git，每次重跑）
