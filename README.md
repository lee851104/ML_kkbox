...
| 挑戰 | 做法 |
|---|---|
| **4.1 億列收聽日誌（30.5 GB）**，pandas 直接 OOM | Polars lazy execution ＋ 讀取階段 predicate pushdown。收斂成每人一列：6,900 萬列 / 2.20 GiB / **32 秒** |
| **標籤藏在資料裡** —— 到期後 30 天的交易就是答案 | as-of 截斷，每欄特徵只由 cutoff 前已發生的事算出。八條防洩漏紅線各有一個「餵違規資料會 raise」的守門測試 |
| **單一切分分不出「模型強」與「這個月剛好合胃口」** | 時間外驗證 Feb→Mar ＋ 反向驗證 Mar→Feb ＋ 配對 8 seed。三方比較選出 CatBoost，優勢 **7.69σ**、配對 **8:0** |
| **挽回優惠必須提前寄出才來得及** | `cutoff = 到期日 − 7 天` 重訓，模型看不到到期日當天的續訂或取消。時間外 Log Loss **0.17921** —— 上線與 Demo 用的都是這一個 |
| **技術指標要能換算成錢** | `E[淨收益] = p × r_save × LTV − C_offer`；投放門檻 `p* = C_offer ÷ (r_save × LTV)` 不看標籤，與期望淨收益曲線的實測極大值差 **NT$ 177（0.005%）** |
| **名單上的每個人都要能解釋為什麼在上面** | CatBoost 原生 TreeSHAP 逐人歸因、組內相加取前三，加總恆等式逐列驗。48,853 人、145,796 句可稽核理由 |
| **上線之後怎麼知道模型還準** | PSI 漂移監控，門檻先量過雜訊地板才定；特徵分布與基準率分開監控 |
| **一個總分會蓋掉整群人** | 每次評估分三群回報。老訂戶流失率 5.87%、首次到期新客 39.84%，**差 6.8 倍** |
| **怎麼確定線上載到的是對的模型** | 模型、特徵欄序、業務假設打包成單一 artifact，三道載入守門（雜湊 / 欄位指紋 / 版本）。存載容差 **0** |
