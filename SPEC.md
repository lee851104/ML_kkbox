...
|---|---|---|---|---|
| `txn_only`（M1） | 23 | 0.16335 | — | — |
| **`log_only`** | 38 | **0.29735** | **+82.0%** | — |
...
| `all`（M2） | 61 | 0.15821 | −3.15% | 0.083% |

**一、收聽行為沒有獨立訊號。** `log_only` 為 0.29735，常數基準為 0.30746 —— 38 個特徵僅勝出 3.3%。單獨使用時，收聽資料幾乎不含可用資訊。

**二、四組高度重疊。** 各組單獨貢獻合計 7.92%，全部併用僅 3.15%，重疊約六成。四組實際上都在衡量同一件事：該用戶是否仍在使用服務。

**三、EDA 的預測與實測相反，這是本階段最重要的方法論教訓。**

依 §7.3 的單變量分析，預期 trend 最強（唯一在兩個 cohort 方向一致者）、intensity 最弱（在 Mar 完全平坦）。實測恰好相反：trend −1.40% 為最弱，intensity −2.91% 為最強。
