...
#
# **這個值換過一次（0.00084 → 0.00048），理由值得記住。**
#
# 初版用的是 Feb cohort 內部 5-fold 的標準差，因為那是當時手上唯一的
# 變異數估計。但它量的是「Feb 內部換一批 fold」，而我們判定的對象是
# **Mar 的 log loss** —— 兩者不是同一個量。
#
# §7.8 的多 seed 實驗順帶提供了正確的估計：同一組特徵、只換
# `inner_split_seed`，Mar log loss 實測為
#     0.15802 / 0.15872 / 0.15864 / 0.15755 / 0.15839
# 標準差 0.00048。這是直接量在**我們實際回報的那個指標**上，因此取代
# 舊值。
#
# ⚠️ 它仍然低估了「換一個月」的變異（只有兩個 cohort，估不出來）。
# 所以這是一個**寬鬆**門檻：連它都跨不過的一定是雜訊，跨過了也不保證
# 跨得過真正的月間變異。
NOISE_SIGMA = 0.00048
